...
func newFlowReqCtx(runID string, ag core.Agent, pm core.PluginManager) core.RequestContext {
...
func TestFlowExecuteModel_ShortCircuit(t *testing.T) {
	...
	ag.DescriptionVal = "flow"
	ag.ModelVal = &testutil.MockModel{}
	rc := newFlowReqCtx("run-short", ag, pm)
	w := &testutil.CollectingWriter{}
	resp, err := ExecuteModel(context.Background(), rc, ag, &core.ModelRequest{}, w)
...
func TestFlowExecuteModel_AfterReplacement(t *testing.T) {
	...
	rc := newFlowReqCtx("run-after", ag, pm)
	w := &testutil.CollectingWriter{}
	resp, err := ExecuteModel(context.Background(), rc, ag, &core.ModelRequest{}, w)
	require.NoError(t, err)
	require.NotNil(t, resp)
...
func TestFlowExecuteModel_OnModelErrorRecovery(t *testing.T) {
	...
	rc := newFlowReqCtx("run-err", ag, pm)
	w := &testutil.CollectingWriter{}
	resp, err := ExecuteModel(context.Background(), rc, ag, &core.ModelRequest{}, w)
	require.NoError(t, err)
	require.NotNil(t, resp)

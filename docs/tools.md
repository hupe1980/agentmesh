...
Tools can have their own guardrails to validate inputs and outputs at the tool level. This provides fine-grained control over individual tool safety, independent of agent-level guardrails.

### Input guardrails

Input guardrails validate tool arguments before execution:
...
// Create a content filter for tool inputs
contentFilter := guardrail.NewContentFilterGuardrail([]string{"hack", "exploit", "attack"})
...
If the input contains blocked content, the tool returns an error before execution.

### Output guardrails

Output guardrails validate tool results after execution:

```go
// Prevent sensitive data in tool outputs
outputFilter := guardrail.NewContentFilterGuardrail(
    []string{"password", "secret", "api_key"},
)

databaseTool, err := tool.NewFuncTool(
    "query_database",
    "Query the database",
    func(ctx context.Context, args QueryArgs) (string, error) {
        return executeQuery(args.SQL)
    },
    tool.WithOutputGuardrails(outputFilter),
)
```

### Combining input and output guardrails

You can apply both input and output guardrails to a single tool:

```go
tool, err := tool.NewFuncTool(
    "process_data",
    "Process user data",
    processFunc,
    tool.WithInputGuardrails(inputFilter, piiDetector),
    tool.WithOutputGuardrails(outputFilter, sensitiveDataFilter),
)
```

### Tripwire behavior

Guardrails can be configured with `ActionRaise` to trigger a tripwire, which returns a `TripwireError` that can be caught for special handling:

```go
// Create a guardrail that raises on detection
injectionDetector := guardrail.NewContentFilterGuardrail(
...
tool, err := tool.NewFuncTool(
    ...
    "Execute SQL query",
    queryFunc,
    tool.WithInputGuardrails(injectionDetector),
)

// Handle tripwire errors
result, err := tool.Call(ctx, args)
if err != nil {
    var tripwire *guardrail.TripwireError
    if errors.As(err, &tripwire) {
        // Log security event, alert, etc.
        log.Printf("Security tripwire: %s", tripwire.Result.Message)
    }
}
```

> **Note:** Tool-level guardrails are independent of [agent-level guardrails](/agents/#guardrails). For comprehensive protection, combine both layers.

...
func TestNamespacedNodeReceivesFilteredView(t *testing.T) {
	...
	// Execute the node
	_, err = agent1Node.Execute(ctx, view)
	require.NoError(t, err)
	...
	_, err = agent2Node.Execute(ctx, view)
	require.NoError(t, err)
...
func TestRegularNodeSeesAllNamespaces(t *testing.T) {
	...
	// Regular node (BaseCommandNode) should see all keys
	var seenKeys []string
	regularNode := &graph.BaseCommandNode{
		NodeName: "regular",
	...
	_, err = regularNode.Execute(ctx, view)
	require.NoError(t, err)

...
// RetryPolicy configures automatic retry behavior for node execution failures.
// It specifies how many times to retry, how long to wait between attempts,
// and which errors should trigger a retry.
type RetryPolicy struct {
	...
	// Backoff is a function that calculates the wait duration before each retry attempt.
	// The function receives the attempt number (1-indexed) and returns the wait duration.
	// If nil, no waiting occurs between retries.
	Backoff func(attempt int) time.Duration

	// Retryable is a function that determines if an error should trigger a retry.
	// If nil, all errors are considered retryable.
	// Use this to implement selective retry logic (e.g., only retry transient errors).
	Retryable func(error) bool
}

// ShouldRetry returns true if the given error should trigger a retry.
...
// GetBackoffDuration returns the wait duration for the given attempt number.

...
func (m *mockModel) Stream(ctx context.Context, messages []message.Message) (*model.Stream, error) {
	...
	go func() {
		defer close(chunks)
		chunks <- model.StreamChunk{Text: "Stream", Final: false}
		chunks <- model.StreamChunk{Text: "ing", Message: m.response, Final: true}
	}()
...
func TestMockModel_Generate(t *testing.T) {
	...
	result, err := mock.Generate(context.Background(), nil)
	if err != nil {
...
func TestMockModel_Stream(t *testing.T) {
	...
	stream, err := mock.Stream(context.Background(), nil)
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
	...
	var collected string
	for stream.Next() {
		collected += stream.Current().Text
	}

	if collected != "Streaming" {
		t.Errorf("Expected 'Streaming', got %q", collected)
	}
}
